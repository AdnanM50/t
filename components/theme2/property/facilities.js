...
const Facilities = ({singleData}) => {
    ...
    return (
        <div className='my-14 bg-[#FAFAFF] p-7 rounded-md'>
    ...
    return (
        <div className='h-full w-full rounded-md bg-white transition-shadow delay-100 ease-in-out hover:shadow-xl py-8'>
            <div className='flex h-full  w-full flex-col items-center justify-center'>
                <div className='flex h-[42px] w-[52px] items-center justify-center'>
                    <img src={icon} alt='' className=' object-contain' />
                </div>
                <p className='header_4_bold mb-2 mt-4 text-dark_text'> {title} </p>
                <p className='paragraph_1 text-secondary_text'>{value}</p>
            </div>
        </div>
    );
};
